...
class Trainer(BaseTrainer):
	...
	def __init__(self, model, criterion, criterion_continuous, metric_ftns, metric_ftns_continuous, optimizer, config, data_loader, categorical=True, continuous=True,
		...
		self.train_metrics = MetricTracker('loss', 'loss_categorical', 'loss_continuous',
			'roc_auc_micro', 'roc_auc_macro', writer=self.writer)
		self.valid_metrics = MetricTracker('loss', 'loss_categorical', 'loss_continuous',
			'roc_auc_micro', 'roc_auc_macro', writer=self.writer)
	...
	def _train_epoch(self, epoch, phase="train"):
		...
		paths = []
		for batch_idx, (data, target, target_continuous, lengths, melspec) in enumerate(data_loader):
